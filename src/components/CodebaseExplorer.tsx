...
export default function CodebaseExplorer({
  ...
  return (
    <div
      className="flex flex-col h-full bg-[#0F1117] border-r border-slate-800 text-slate-300 font-sans"
    ...
    >
      ...
      <div className="p-4 border-b border-slate-800 flex items-center justify-between bg-[#0F1117]">
        ...
        <div className="flex items-center gap-1">
          <button
            ...
            className="p-1 hover:bg-slate-800 hover:text-indigo-400 rounded text-slate-500 transition"
      ...
      {/* Add New File Form */}
      {showAddForm && (
        <form onSubmit={handleCreateFile} className="p-3 bg-slate-950 border-b border-slate-800 flex gap-2">
          <input
            type="text"
            placeholder="filename.js..."
            value={newFileName}
            onChange={(e) => setNewFileName(e.target.value)}
            className="flex-1 px-2.5 py-1 text-xs bg-slate-900 border border-slate-700 rounded text-slate-200 focus:outline-none focus:border-indigo-500"
            autoFocus
          />
          <button
            type="submit"
            className="px-3 py-1 bg-indigo-600 text-slate-100 text-xs rounded font-medium hover:bg-indigo-500 transition"
          >
            Create
          </button>
        </form>
      ...
      <div
        className={`p-4 border-b border-slate-800/80 bg-slate-900/10 flex flex-col gap-2 transition ${
      ...
      >
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <button
            ...
            className="flex items-center justify-center gap-2 p-2 bg-slate-900/60 hover:bg-slate-850 border border-slate-800 rounded text-slate-300 text-[11px] font-medium transition cursor-pointer"
          >
        ...
        {Object.entries(tree).map(([dir, dirFiles]) => {
          ...
          return (
            <div key={dir} className="space-y-1">
              ...
              <div className={isRoot ? '' : 'pl-3 border-l border-slate-800/60 space-y-0.5'}>
                {dirFiles.map((file) => {
                  const isActive = file.id === activeFileId;
                  return (
                    <div
                      key={file.id}
                      className={`group flex items-center justify-between px-2.5 py-1.5 rounded transition text-xs ${
                        isActive
      ...
      <div className="p-4 bg-slate-900/20 border-t border-slate-800">
        ...
        <div className="flex h-12 gap-1 items-end pt-1">
          <div className="flex-1 h-6 bg-slate-800/85 rounded-t-sm hover:bg-indigo-500 transition-all duration-300 cursor-pointer" title="Workspace Clean"></div>
          <div className="flex-1 h-8 bg-slate-800/85 rounded-t-sm hover:bg-indigo-500 transition-all duration-300 cursor-pointer" title="Direct Code Sync"></div>
          <div className="flex-1 h-4 bg-slate-800/85 rounded-t-sm hover:bg-indigo-500 transition-all duration-300 cursor-pointer" title="Local Snapshots"></div>
          <div className="flex-1 h-10 bg-indigo-500 rounded-t-sm animate-pulse cursor-pointer" title="AI Ingest Activity"></div>
          <div className="flex-1 h-2 bg-slate-800/85 rounded-t-sm hover:bg-indigo-500 transition-all duration-300 cursor-pointer" title="Initial Import"></div>
          <div className="flex-1 h-5 bg-slate-800/85 rounded-t-sm hover:bg-indigo-500 transition-all duration-300 cursor-pointer" title="Tunnel Diagnostic"></div>
